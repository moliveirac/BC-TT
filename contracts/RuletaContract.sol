...
contract LaRuleta is Ownable {
    ...
    event TiradaCompletada(uint8 resultado);
    ...
    function _hacerApuesta(Numero memory _numero, address payable jugador, uint cantidad) private returns (uint, Numero memory) {
        require(cantidad > 0, "You must provide some ETH");

        Numero memory resultado = _girarRuleta();

        return (_revisarTipoApuesta(Apuesta(payable(jugador), _numero, cantidad), resultado), resultado);
    }

    function hacerApuestaNumero(uint8 _numero) public payable {
        require(_numero >= 0 && _numero <= 37, "Invalid number");
        require(msg.value * 36 <= address(this).balance, "Can not pay if you win");

        uint premio;
        Numero memory premiado;
        (premio, premiado) = _hacerApuesta(Numero(_numero, Paridad.NONE, Color.VERDE, Docena.NONE), payable (msg.sender), msg.value);
        ...
        emit TiradaCompletada(premiado.numero);
    }

    function hacerApuestaColor(uint8 _color) public payable {
        require(_color >= 0 && _color <= 1, "Invalid number");
        require(msg.value * 2 <= address(this).balance, "Can not pay if you win");
        ...
        uint premio;
        Numero memory premiado;
        (premio, premiado) = _hacerApuesta(Numero(38, Paridad.NONE, apuestaColor, Docena.NONE), payable (msg.sender), msg.value);
    ...
    function hacerApuestaParidad(uint8 _paridad) public payable {
        ...
        uint premio;
        Numero memory premiado;
        (premio, premiado) = _hacerApuesta(Numero(38, apuestaParidad, Color.VERDE, Docena.NONE), payable (msg.sender), msg.value);
    ...
    function hacerApuestaDocena(uint8 _docena) public payable {
        ...
        require(msg.value * 3 <= address(this).balance, "Can not pay if you win");
        ...
        uint premio;
        Numero memory premiado;
        (premio, premiado) = _hacerApuesta(Numero(38, Paridad.NONE, Color.VERDE, apuestaDocena), payable (msg.sender), msg.value);
